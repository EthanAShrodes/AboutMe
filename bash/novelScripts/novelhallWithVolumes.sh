...
do
	...
	if [ "$check" = "Chapter $chapnum" ]; then 
        ...
        #start=`cat currentchapt.txt | grep '<a title="v[0-9] Chapter ' | grep 'rel="next">Next' | grep -o '/${parts[3]}/[0-9]\+.html'`
		start=`cat currentchapt.txt | grep '<a title="v[0-9] Chapter ' | grep 'rel="next">Next' | grep "/${parts[3]}" | grep -o '/[0-9]\+.html'`
		...
		echo "https://${parts[2]}/${parts[3]}$start"
		echo ""
		start=`cat currentchapt.txt | grep '<a title="' | grep 'rel="next">Next' | grep "/${parts[3]}" | grep -o '/[0-9]\+.html'`
		sleep 4
		curl "https://${parts[2]}/${parts[3]}$start" > currentchapt.txt
		date +'%I:%M%p'
		#notify-send -u critical -t 0 "script error" "It failed at Chapter $chapnum"
	fi
done
notify-send -u critical -t 0 "script done" "<a href='https://novelhall.com$start'>next chapter</a>"
